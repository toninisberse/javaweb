...
<%
  ...
  if(email == "" || senha == ""){//Verifica se possui campos em branco
    ...
    try{
        ...
        rs = st.executeQuery("SELECT *FROM clientes WHERE usuario = '"+email+"' AND senha = '"+senha+"' ");
        while(rs.next()){
           user = rs.getString("usuario");//Armazenando o email do banco
           pass = rs.getString("senha");//Armazenando a senha do banco
           NomeUser = rs.getString("nome");//Armazenando o nome do banco
        }
